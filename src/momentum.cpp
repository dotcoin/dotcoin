...
//    std::cerr<<"result_hash "<<a<<"  "<<a%8<<"  --- ";
 ...
 //   std::cerr<<"    "<<getBirthdayHash(head,a)<<"   "<<getBirthdayHash(head,b)<<"\n";
        if( a == b )
        {
            printf("bts::momentum_verify( midHash, nBirthdayA, nBirthdayB) a == b\n");
            return false;
        }
        if( a > MAX_MOMENTUM_NONCE ) {
            printf("bts::momentum_verify( midHash, nBirthdayA, nBirthdayB) a > MAX_MOMENTUM_NONCE\n");
            return false;
        }
        if( b > MAX_MOMENTUM_NONCE ) {
            printf("bts::momentum_verify( midHash, nBirthdayA, nBirthdayB) b > MAX_MOMENTUM_NONCE\n");
            return false;
        }
