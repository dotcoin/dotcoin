...
// Settings
int64 nTransactionFee = 0.001*COIN;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    {
        nSubsidy = 8 * COIN;
        int exponent=((nHeight-2048) / 65536);
        ...
        if(nSubsidy<1*COIN){nSubsidy=1*COIN;}
    }
...
static const int64 nTargetTimespan = 32 * 60; // 32 minutes
static const int64 nTargetSpacing = 1 * 60; // five minutes
...
//    fprintf( stderr, "init hash %s\n", r.ToString().c_str() );
		if(!bts::momentum_verify( midHash, nBirthdayA, nBirthdayB)){
            //printf("bts::momentum_verify( midHash, nBirthdayA, nBirthdayB) return false\n");
